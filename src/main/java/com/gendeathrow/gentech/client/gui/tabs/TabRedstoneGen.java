...
public class TabRedstoneGen extends TabBase 
{
		...
		public TabRedstoneGen(GuiBase gui, IRedstoneControl container) {

			this(gui, defaultSide, container);
		}

		public TabRedstoneGen(GuiBase gui, int side, IRedstoneControl container) {
		...
		public void draw() {
			...
			drawBackground();
			gui.drawIcon(IconRegistry.getIcon("IconRedstone"), posXOffset(), posY + 3, 1);
			if (!isFullyOpened()) {
				return;
			}
			...
			if (myContainer.getControl().isDisabled()) {
				gui.drawButton(IconRegistry.getIcon("IconGunpowder"), posX() + 28, posY + 20, 1, 1);
				gui.drawButton(IconRegistry.getIcon("IconRSTorchOff"), posX() + 48, posY + 20, 1, 0);
				gui.drawButton(IconRegistry.getIcon("IconRSTorchOn"), posX() + 68, posY + 20, 1, 0);
				...
				if (myContainer.getControl().isLow()) {
					gui.drawButton(IconRegistry.getIcon("IconRedstone"), posX() + 28, posY + 20, 1, 0);
					gui.drawButton(IconRegistry.getIcon("IconRSTorchOff"), posX() + 48, posY + 20, 1, 1);
					gui.drawButton(IconRegistry.getIcon("IconRSTorchOn"), posX() + 68, posY + 20, 1, 0);
					getFontRenderer().drawString(StringHelper.localize("info.cofh.low"), posXOffset() + 14, posY + 78, textColor);
				} else {
					gui.drawButton(IconRegistry.getIcon("IconRedstone"), posX() + 28, posY + 20, 1, 0);
					gui.drawButton(IconRegistry.getIcon("IconRSTorchOff"), posX() + 48, posY + 20, 1, 0);
					gui.drawButton(IconRegistry.getIcon("IconRSTorchOn"), posX() + 68, posY + 20, 1, 1);
		...
		public boolean onMousePressed(int mouseX, int mouseY, int mouseButton) {
			...
			if (28 <= mouseX && mouseX < 44 && 20 <= mouseY && mouseY < 36) {
				if (!myContainer.getControl().isDisabled()) {
					...
					Minecraft.getMinecraft().playerController.sendEnchantPacket(this.gui.inventorySlots.windowId, 15);
				}
			} else if (48 <= mouseX && mouseX < 64 && 20 <= mouseY && mouseY < 36) {
				if (!myContainer.getControl().isLow()) {
					myContainer.setControl(IRedstoneControl.ControlMode.LOW);
					Minecraft.getMinecraft().playerController.sendEnchantPacket(this.gui.inventorySlots.windowId, 16);
				...
				if (!myContainer.getControl().isHigh()) {
					...
					Minecraft.getMinecraft().playerController.sendEnchantPacket(this.gui.inventorySlots.windowId, 17);
